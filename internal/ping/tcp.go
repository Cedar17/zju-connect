...
// StartContext starts the probe loop with a caller-owned cancellation source.
// Cancelling ctx closes a currently dialing or handshaking probe instead of
// waiting for its local timeout.
...
// Stop interrupts the current probe, including an in-flight dial or TLS
// handshake. It is safe to call repeatedly and before Start.
...
func (tcping *TCPing) pingContext(parent context.Context) (time.Duration, net.Addr, error) {
	...
	tlsConn := tls.Client(conn, &tls.Config{
		...
		InsecureSkipVerify: true, // #nosec G402 -- this is a reachability probe, not an authenticated data path.
	})
